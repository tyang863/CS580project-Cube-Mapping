...
int Application5::Initialize()
{
	...
	int xs, ys;
	std::string _box = BOX;
	std::string box = _box + "_ft.ppm";

/***************** 此处作用是事先读取cube map的边长，以便程序工作 *******************/
	fd = fopen(box.c_str(), "rb");
	if (fd == NULL) {
...
/************************************************************************/
	 ...
	 */ 
 	m_nWidth = xs;		// frame buffer and display width
	m_nHeight = xs;    // frame buffer and display height

/******************如果要查看cube map，则取消以下两行的注释********************/
	//m_nWidth = xs * 3;
	//m_nHeight = ys * 2;
/************************************************************************/
...
/******************如果要查看cube map，则注释掉以下三行********************/
	     valueListTriangle[0] = (GzPointer)vertexList; 
		 valueListTriangle[1] = (GzPointer)normalList; 
		 m_pRender->GzPutOriginTriangle(valueListTriangle); 
/*********************************************************************/
	}

/******************如果要查看cube map，则注释掉以下三行********************/
	m_pRender->GzParseTriangles();
...
/*********************************************************************/
	...
	//FILE *output;
	//output = fopen("range.txt", "w");
	//fprintf(output, "Xmin: %f Xmax: %f\n", m_pRender->coord_min[X], m_pRender->coord_max[X]);
	//fprintf(output, "Ymin: %f Ymax: %f\n", m_pRender->coord_min[Y], m_pRender->coord_max[Y]);
	//fprintf(output, "Zmin: %f Zmax: %f\n", m_pRender->coord_min[Z], m_pRender->coord_max[Z]);

	//fclose(output);

/******************如果要查看cube map，则取消以下三行的注释********************/
	//m_pRender->GzShowCubeMap();
	//m_pRender->GzFlushDisplay2File(outfile); 	/* write out or update display to file*/
	//m_pRender->GzFlushDisplay2FrameBuffer();	// write out or update display to frame buffer
/************************************************************************/
